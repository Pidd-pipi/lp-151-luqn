...
- 内容编辑与撤回：作者可在详情页编辑/撤回自己的帖子和评论；编辑后重新检测敏感词，命中转入审核；帖子撤回时评论、点赞、标签关系一并撤下，评论撤回后帖子评论数同步减少
...
| GET | /posts/{id} | 帖子详情（待审/未通过仅作者可见，已撤回返回 404） |
| PUT | /posts/{id} | 编辑自己的帖子（需登录，命中敏感词转入审核） |
| DELETE | /posts/{id} | 撤回自己的帖子（需登录，评论/点赞/标签一并撤下） |
| GET | /posts/{id}/comments | 帖子评论 |
| POST | /comments | 发表评论（需登录） |
| PUT | /comments/{id} | 编辑自己的评论（需登录，命中敏感词转入审核） |
| DELETE | /comments/{id} | 撤回自己的评论（需登录，帖子评论数同步减少） |
| GET | /tags | 标签列表 |
| POST | /tags | 创建标签（需登录） |
