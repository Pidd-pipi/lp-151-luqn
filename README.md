...
- 匿名发帖与评论：随机昵称 + 唯一身份密钥，凭密钥管理自己的内容（编辑/撤回），图文帖 + 匿名评论
- 内容编辑与撤回：作者可在详情页编辑或撤回自己的帖子/评论，非本人请求返回 403；编辑后重新检测敏感词，命中先进入审核并仅作者可见
- 撤回落链：撤回帖子时其评论、点赞、标签关系一起撤下，列表与详情不再出现；撤回评论时帖子评论数同步减少
- 审核状态可见：作者可在详情页看到「审核中 / 已放行 / 已拒绝 / 已取消」处理状态与备注，审核放行或拒绝后内容与状态实时变化
...
| GET | /posts/{id} | 帖子详情（撤回不可见；审核中/屏蔽仅作者可见） |
| PUT | /posts/{id} | 作者编辑帖子，重新检测敏感词（需登录） |
| POST | /posts/{id}/withdraw | 作者撤回帖子，级联撤下评论/点赞/标签（需登录） |
| GET | /posts/{id}/comments | 帖子评论（公开评论 + 本人未撤回评论） |
| POST | /comments | 发表评论（需登录） |
| PUT | /comments/{id} | 作者编辑评论，重新检测敏感词（需登录） |
| POST | /comments/{id}/withdraw | 作者撤回评论，帖子评论数同步减少（需登录） |
| GET | /tags | 标签列表 |
| POST | /tags | 创建标签（需登录） |
